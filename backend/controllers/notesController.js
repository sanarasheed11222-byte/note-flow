...
const getNotes = async (req, res, next) => {
  try {
    ...
    let query = 'SELECT id, title, content, category, tags, created_at, updated_at FROM notes WHERE user_id = ?';
...
const updateNote = async (req, res, next) => {
  try {
    const [existing] = await pool.query('SELECT id FROM notes WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
    if (existing.length === 0) return res.status(404).json({ status: 'error', message: 'Note not found' });
    const { title, content, category, tags } = req.body;
    const tagsJSON = tags ? JSON.stringify(tags) : JSON.stringify([]);
    await pool.query(
